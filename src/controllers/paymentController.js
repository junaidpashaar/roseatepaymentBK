...
class PaymentController {
  ...
  async generateDepositPaymentLink(req, res, next) {
    try {
      let body = { guestName:req?.body?.guestName, name:req?.body?.hotelId, reservationId:req?.body?.reservationId, type:req?.body?.type, amount:req?.body?.amount, description:req?.body?.policyIds,info:JSON.stringify(req.body) };
